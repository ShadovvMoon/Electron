...
void BSP::setup(ProtonMap *map, ProtonTag *scenario, Pipeline *pipeline) {
    ...
    vao->setup();
    vao->vertex_array    = (float*)malloc(vertex_size * 3 * sizeof(float));
    vao->texture_uv      = (float*)malloc(vertex_size * 2 * sizeof(float));
    vao->light_uv        = (float*)malloc(vertex_size * 2 * sizeof(float));
    vao->normals         = (float*)malloc(vertex_size * 3 * sizeof(float));
    vao->tangents        = (float*)malloc(vertex_size * 3 * sizeof(float));
    vao->binormals       = (float*)malloc(vertex_size * 3 * sizeof(float));
    vao->index_array     = (int*)malloc(index_size * sizeof(int));
    ...
    for (int i = 0; i < renderables.size(); i++) {
        BSPRenderMesh *rmesh = renderables[i];
        for (int s = 0; s < rmesh->submeshes.size(); s++) {
            BSPRenderSubmesh *submesh = rmesh->submeshes[s];
            Submesh *sm = mesh->addSubmesh(
                             submesh->shader,
                             submesh->vertexOffset,
                             submesh->vertCount,
                             submesh->indexOffset,
                             submesh->indexCount);
            sm->bindTexture(3, rmesh->lightTexture, submesh->lightmap);
        }
    }
#else
