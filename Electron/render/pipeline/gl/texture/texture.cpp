...
// NORMAL TEXTURE
gltexture::gltexture(ProtonMap *map, HaloTagDependency bitm) {
...
void gltexture::bind(Pipeline *pipeline) {
    // TODO: Compressed with Color-Key Transparency [black remove!]
    if (textures.size() > 0) {
        glBindTexture(GL_TEXTURE_2D, textures[0]);
    }
}
void gltexture::bind(int i, Pipeline *pipeline) {
	if (i >= 0 && i < textures.size()) {
		glBindTexture(GL_TEXTURE_2D, textures[i]);
	}
}

void gltexture_cubemap::load_side(GLenum side_target, GLenum i, ProtonTag *bitmapTag) {
    bitm_header_t *bitmData = (bitm_header_t *)bitmapTag->Data();
...
// CUBEMAP
gltexture_cubemap::gltexture_cubemap(ProtonMap *map, HaloTagDependency bitm) {
...
void gltexture_cubemap::bind(Pipeline *pipeline) {
    glBindTexture(GL_TEXTURE_CUBE_MAP, tex);
}

texture *GLTextureManager::create_texture(ProtonMap *map, HaloTagDependency bitm) {
    ...
    texture *tex = new gltexture(map, bitm);
    textures[bitm.tag_id.tag_index] = tex;
    return tex;
}

texture_cubemap *GLTextureManager::create_cubemap(ProtonMap *map, HaloTagDependency bitm) {
    printf("%d\n", bitm.tag_id.tag_index);
    ...
    // Create a new texture
    texture_cubemap *tex = new gltexture_cubemap(map, bitm);
