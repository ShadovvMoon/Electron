...
void main( void )
{
    ...
    vec4 sum = vec4(0.0);
    sum += texture2D(tDepth, vec2(vTexCoord.x - 4.0*blursize, vTexCoord.y)) * 0.05;
    sum += texture2D(tDepth, vec2(vTexCoord.x - 3.0*blursize, vTexCoord.y)) * 0.09;
    sum += texture2D(tDepth, vec2(vTexCoord.x - 2.0*blursize, vTexCoord.y)) * 0.12;
    sum += texture2D(tDepth, vec2(vTexCoord.x - blursize, vTexCoord.y)) * 0.15;
    sum += texture2D(tDepth, vec2(vTexCoord.x, vTexCoord.y)) * 0.16;
    sum += texture2D(tDepth, vec2(vTexCoord.x + blursize, vTexCoord.y)) * 0.15;
    sum += texture2D(tDepth, vec2(vTexCoord.x + 2.0*blursize, vTexCoord.y)) * 0.12;
    sum += texture2D(tDepth, vec2(vTexCoord.x + 3.0*blursize, vTexCoord.y)) * 0.09;
    sum += texture2D(tDepth, vec2(vTexCoord.x + 4.0*blursize, vTexCoord.y)) * 0.05;
    gl_FragColor = mix(image / 2, image, sum.x);
