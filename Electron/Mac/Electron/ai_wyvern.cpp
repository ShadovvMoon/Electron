...
float weapRange(ProtonMap *map, ProtonTag *tag) {
    ...
    if (data) {
        ...
        for (i=0; i < triggers.count; i++) {
            HaloWeaponTrigger *trigger = (HaloWeaponTrigger *)(tag2mem(tag, (uint32_t)(uint64_t)triggers.pointer) + i * sizeof(HaloWeaponTrigger));
            if (trigger) {
                if (trigger->projectile.identity.tagTableIndex != NULLED_TAG_ID) {
                    ProtonTag *projTag = map->tags.at(trigger->projectile.identity.tagTableIndex).get();
                    if (projTag) {
